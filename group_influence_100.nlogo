...
to init-info
  if initial_info_distribution = "random" [
    ask turtles [
      ...
      set size (value / 100) + 1 ; so that a node is not invisible if value = 0
    ]
  ]
  if initial_info_distribution = "normal" [
    ask turtles [
      set value random-normal 50 34.1
      set size (value / 100) + 1 ; so that a node is not invisible if value = 0
    ]
  ]
end
...
to check-spread
  ...
  ask turtles [
    let check_val value
    set same_as_neighbors? all? link-neighbors [ value = check_val ]
  ]
  ...
  if ticks mod meeting_frequency = 0 [
    foreach groups [ members ->
      ...
      if group_action = "average" [
        ;; everyone adopts average value of members
        let num_members (count members)
        ask members [
          set group_value (group_value + value)
        ]
        set group_value (group_value / num_members)
      ]
...
313
380
individual_action: if a node is going to be influenced by its neighbor, does it copy its neighbor or compromise with its neighbor
11
0.0
1
...
310
186
initial_info_distribution: given 100 possible values for a node to claim, does a node claim a value randomly or from a normal distribution with a mean value of 50
11
0.0
1
